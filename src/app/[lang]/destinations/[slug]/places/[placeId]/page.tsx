  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        type: 'article',
        url: `https://morocompase.com/destinations/${slug}/places/${placeId}`,
      },
      twitter: {
      ...
      alternates: {
        canonical: `https://morocompase.com/destinations/${slug}/places/${placeId}`,
      },
    };
  } catch (error) {
...
export default async function PlacePage({ params }: PlacePageProps) {
  ...
  try {
    ...
    const place = await getPlaceById(destination.id, placeId);
    if (!place) {
      console.log(`Place not found for ID: ${placeId} in destination: ${destination.id}`);
      notFound();
    }
    ...
    if (!convertedPlace.name || !convertedPlace.description) {
      console.error('Invalid place data structure:', convertedPlace);
      notFound();
    }
