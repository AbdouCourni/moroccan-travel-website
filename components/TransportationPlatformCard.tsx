// components/TransportationPlatformCard.tsx
'use client';

import Image from 'next/image';
import { ExternalLink, Star, Train, Bus, Car, Plane } from 'lucide-react';
import { TransportationPlatform } from '../types';

interface TransportationPlatformCardProps {
  platform: TransportationPlatform;
  destinationName?: string;
}

export function TransportationPlatformCard({ platform, destinationName }: TransportationPlatformCardProps) {
  ...
  const getTransportIcon = (type: string) => {
    switch (type) {
      ...
      case 'car_rental': return <Car className="w-3 h-3" />;
  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
      ...
      <div className="p-6">
        ...
        {/* Transport Types */}
        {platform.transportTypes && platform.transportTypes.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {platform.transportTypes.map((type, index) => (
        ...
        <button
          ...
          className="w-full bg-primary-gold text-black py-3 rounded-xl font-semibold hover:bg-opacity-90 transition-all duration-300 transform hover:scale-105 shadow-md flex items-center justify-center gap-2"
